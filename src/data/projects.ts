...

{
...

  {
    ...
    content: {
      fr: "Après le blog, j'ai étendu le projet à la landing page de Julaya (julaya.co), jusque-là statique, pour la rendre entièrement pilotable depuis le CMS. J'ai modélisé dans Strapi les différentes pages (accueil, produits, tarifs, à propos, contact...) et une cinquantaine de composants de section réutilisables (hero, cartes de fonctionnalités, grilles de tarifs, avis, appels à l'action).\n\nL'équipe marketing peut ainsi composer et faire évoluer ses pages bloc par bloc, en toute autonomie. Le front Next.js (App Router, SSR) consomme ces données, gère le multilingue (next-intl) et le rendu serveur pour le référencement.",
      en: "After the blog, I extended the project to Julaya's landing page (julaya.co), until then static, to make it fully CMS-driven. In Strapi I modeled the various pages (home, products, pricing, about, contact...) and around fifty reusable section components (hero, feature cards, pricing grids, reviews, calls to action).\n\nThe marketing team can now compose and evolve its pages block by block, fully autonomously. The Next.js front end (App Router, SSR) consumes this data, handles multilingual content (next-intl) and server-side rendering for SEO.",
    },
    roles: ["Front-end"],
    stack: ["Next.js (App Router SSR) · React · Sass · next-intl · Strapi (CMS)"],
...

  {
    ...
    content: {
      fr: "Développement du blog public de Julaya (blog.julaya.co) à partir d'une maquette Figma, dans le cadre de mon alternance. J'ai modélisé les contenus dans Strapi - articles, auteur·ices, catégories et leurs relations - et exposé l'API correspondante.\n\nLe front, intégré en Next.js (App Router, rendu SSR) et connecté au CMS, affiche les articles dynamiquement. L'objectif : rendre l'équipe marketing autonome pour rédiger et publier sans passer par l'équipe technique, sur une base claire et évolutive, avec un rendu côté serveur favorable au référencement.",
      en: "Development of Julaya's public blog (blog.julaya.co) from a Figma mockup, during my apprenticeship. I modeled the content in Strapi - articles, authors, categories and their relations - and exposed the matching API.\n\nThe front end, built with Next.js (App Router, SSR) and connected to the CMS, displays articles dynamically. The goal: make the marketing team autonomous to write and publish without going through the technical team, on a clear and scalable base, with server-side rendering that helps SEO.",
    },
    roles: ["Front-end", "Content integration"],
    stack: ["Next.js (App Router SSR) · React · Sass · Strapi (CMS)"],
...

  {
    ...
    subtitle: {
      fr: "Prototype d'app de dessin iOS (SwiftUI), pensée autour d'une toile collective. (projet école)",
      en: "iOS drawing app prototype (SwiftUI), designed around a collective canvas. (school project)",
    },
    content: {
      fr: "Draw Together est un prototype d'application de dessin iOS développé en SwiftUI, l'une de mes premières explorations de l'écosystème. L'idée de départ : une toile collective où chacun contribue à une œuvre commune.\n\nL'app en pose les fondations côté dessin : une zone de tracé à main levée (SwiftUI Canvas et gestures), avec choix de la couleur et de l'épaisseur du trait, une barre d'onglets personnalisée et animée (accueil, favoris, enregistrements, profil) et un bouton d'action central. La dimension collaborative et le partage en réseau restent à l'état d'intention : ce projet m'a surtout servi à prendre en main SwiftUI (Canvas, gestion des gestes, état, navigation).",
      en: "Draw Together is an iOS drawing app prototype built with SwiftUI, one of my first explorations of the ecosystem. The starting idea: a collective canvas where everyone contributes to a shared artwork.\n\nThe app lays the drawing foundations: a freehand drawing area (SwiftUI Canvas and gestures), with color and stroke-width selection, a custom animated tab bar (home, liked, records, profile) and a central action button. The collaborative, networked side remains an intention: this project mainly served to get hands-on with SwiftUI (Canvas, gesture handling, state, navigation).",
    },
    roles: ["iOS"],
